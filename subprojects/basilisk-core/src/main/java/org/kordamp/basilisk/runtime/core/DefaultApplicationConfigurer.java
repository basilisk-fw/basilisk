...
import org.kordamp.basilisk.runtime.core.controller.NoopActionManager;
...
public class DefaultApplicationConfigurer implements ApplicationConfigurer {
    ...
    private static final String KEY_BASILISK_CONTROLLER_ACTION_HANDLER_ORDER = "basilisk.controller.action.handler.order";
    ...
    @Inject
    public DefaultApplicationConfigurer(@Nonnull BasiliskApplication application) {
    ...
    protected void initializeActionManager() {
        ...
        application.getEventRouter().addEventListener(ApplicationEvent.NEW_INSTANCE.getName(), new RunnableWithArgs() {
            public void run(@Nullable Object... args) {
                ...
                if (BasiliskController.class.isAssignableFrom(klass)) {
                    application.getActionManager().createActions((BasiliskController) args[1]);
                }
            }
        });
        ...
        List<String> handlerOrder = application.getConfiguration().get(KEY_BASILISK_CONTROLLER_ACTION_HANDLER_ORDER, Collections.<String>emptyList());

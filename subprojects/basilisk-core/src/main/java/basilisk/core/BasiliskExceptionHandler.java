...
public class BasiliskExceptionHandler implements ExceptionHandler {
    ...
    private static final String SANITIZED_STACKTRACE_MSG = "Stacktrace was sanitized. Set System property '" + BASILISK_FULL_STACKTRACE + "' to 'true' for full report.";
    ...
    public static boolean isFullStacktraceEnabled() {
        return Boolean.getBoolean(BASILISK_FULL_STACKTRACE);
    }

    private static void printStacktrace(Throwable throwable) {
        if (isOutputEnabled()) {
            if (!isFullStacktraceEnabled()) {
                System.err.println(BASILISK_FULL_STACKTRACE);
            }

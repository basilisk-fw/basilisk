...
public class MethodDescriptor implements Comparable<MethodDescriptor> {
    ...
    private static boolean areParametersCompatible(String[] params1, String[] params2) {
        ...
        for (int i = 0; i < params1.length; i++) {
            String p1 = params1[i];
            String p2 = params2[i];
            if (!p1.equals(p2) && !BasiliskClassUtils.isMatchBetweenPrimitiveAndWrapperTypes(p1, p2)) {
                return false;
            }
        }
    ...
    public boolean matches(@Nonnull MethodDescriptor md) {
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            ...
            if (param1 != null && param2 != null && !BasiliskClassUtils.isAssignableOrConvertibleFrom(param1, param2)) {
                return false;
            }
        }
    ...
    private Class<?> loadClass(@Nonnull String classname) {
        try {
            ...
            if (BasiliskClassUtils.PRIMITIVE_TYPE_COMPATIBLE_TYPES.containsKey(classname)) {
                try {
                    classname = BasiliskClassUtils.PRIMITIVE_TYPE_COMPATIBLE_TYPES.get(classname);
                    return Class.forName(classname, true, getClass().getClassLoader());

...
class ImagePropertyEditorSpec extends Specification {
    @Shared
    private Image sharedImage = new Image(getClass().getClassLoader().getResourceAsStream('basilisk-icon-16x16.png'))
    ...
    void "Valid image format '#format'"() {
        ...
        where:
        format << [
            new File(basePath, 'basilisk-icon-16x16.png'),
            new File(basePath, 'basilisk-icon-16x16.png').toURI(),
            new File(basePath, 'basilisk-icon-16x16.png').toURI().toURL(),
            new FileInputStream(new File(basePath, 'basilisk-icon-16x16.png'))
        ]
    }

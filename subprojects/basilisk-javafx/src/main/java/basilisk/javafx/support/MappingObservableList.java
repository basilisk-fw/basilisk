...
public class MappingObservableList<T, S> extends TransformationList<T, S> {
    ...
    public MappingObservableList(@Nonnull ObservableList<? extends S> source, @Nonnull ObservableValue<Function<S, T>> mapper) {
        ...
        mapper.addListener(new ChangeListener<Function<S, T>>() {
            @Override
            public void changed(ObservableValue<? extends Function<S, T>> v, Function<S, T> o, Function<S, T> n) {
                MappingObservableList.this.updateAll();
            }
        });
    }

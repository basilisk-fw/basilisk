    ...
    public IndexedCardPane() {
                ...
                while (c.next()) {
                    if (c.wasAdded()) {
                        ...
                        if (lastAddedNode != null) {
                            IndexedCardPane.this.doShow(IndexedCardPane.this.indexOf(lastAddedNode));
                        }
                    } else if (c.wasRemoved()) {
                        int selectedIndex = IndexedCardPane.this.getSelectedIndex();
                        for (Node node : c.getRemoved()) {
                            if (!nodes.contains(node)) {
                                continue;
                            }

                            int removedIndex = IndexedCardPane.this.indexOf(node);
                            nodes.remove(node);
                        ...
                        IndexedCardPane.this.doShow(selectedIndex);
                    }
                }
            }
        });
        ...
        widthProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                IndexedCardPane.this.updateBoundsInChildren();
            }
        });
    ...
    protected void doShow(final int index) {
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                if (index < 0) {
                    adjusting.set(true);
                    IndexedCardPane.this.getChildren().clear();
                    adjusting.set(false);
                    selectedIndex.set(-1);
                    selectedNode.set(null);
                } else {
                    ...
                    adjusting.set(true);
                    IndexedCardPane.this.getChildren().setAll(node);
                    adjusting.set(false);

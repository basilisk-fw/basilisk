...
public class NamedCardPane extends StackPane {
    ...
    public NamedCardPane() {
                ...
                while (c.next()) {
                    if (c.wasAdded()) {
                        ...
                        if (lastAddedNode != null) {
                            ...
                            NamedCardPane.this.doShow(nextId);
                        }

                    } else if (c.wasRemoved()) {
                        String selectedId = NamedCardPane.this.getSelectedNodeId();
        ...
        widthProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable, Number oldValue, Number newValue) {
                NamedCardPane.this.updateBoundsInChildren();
            }
        });
    ...
    protected void updateBoundsInChildren() {
        nodes.values().forEach(new Consumer<Node>() {
            @Override
            public void accept(Node node) {
                NamedCardPane.this.updateChildBounds(node);
            }
        });
        layout();
    }
    ...
    protected void doShow(@Nullable final String id) {
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                if (isBlank(id)) {
                    adjusting.set(true);
                    NamedCardPane.this.getChildren().clear();
                    adjusting.set(false);
                    selectedNodeId.set(null);
                    selectedNode.set(null);
                } else {
                    ...
                    adjusting.set(true);
                    NamedCardPane.this.getChildren().setAll(node);
                    adjusting.set(false);

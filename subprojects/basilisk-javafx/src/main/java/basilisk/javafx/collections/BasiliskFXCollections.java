...
import basilisk.javafx.beans.binding.UIThreadAware;
        ...
        protected void sourceChanged(@Nonnull final ListChangeListener.Change<? extends E> c) {
            if (Platform.isFxApplicationThread()) {
                ...
                Platform.runLater(new Runnable() {
                    @Override
                    public void run() {
                        UIThreadAwareObservableList.this.fireChange(c);
                    }
                });
            }
        }
        ...
        protected void sourceChanged(@Nonnull final SetChangeListener.Change<? extends E> c) {
            if (Platform.isFxApplicationThread()) {
                ...
                Platform.runLater(new Runnable() {
                    @Override
                    public void run() {
                        UIThreadAwareObservableSet.this.fireChange(c);
                    }
                });
            }
        }
        ...
        protected void sourceChanged(@Nonnull final MapChangeListener.Change<? extends K, ? extends V> c) {
            if (Platform.isFxApplicationThread()) {
                ...
                Platform.runLater(new Runnable() {
                    @Override
                    public void run() {
                        UIThreadAwareObservableMap.this.fireChange(c);
                    }
                });
            }
        }

...
if (tmplQualifiers) {
    ...
    while (!(artifactBaseName in templateNames)) {
        artifactBaseName = ask("Which type of artifact do you want to generate? ", null, "type")?.toLowerCase().trim()
    }
}
...
processArtifact = { artifactClass, artifactTemplate, artifactType, artifactPath ->
    ...
    if (artifactTemplate == 'View') {
        ...
        destFile = new File(projectDir, concat(concat('basilisk-app/resources', packagePath), filename))
        destFile.parentFile.mkdirs()
...
if (artifactBaseName == 'mvcgroup') {
    ...
    processArtifact(className, 'Model', 'model', 'basilisk-app/models')
    processArtifact(className, 'View', 'view', 'basilisk-app/views')
    processArtifact(className, 'Controller', 'controller', 'basilisk-app/controllers')
    ...
    println "Do not forget to add the group '$groupName' to basilisk-app/conf/Config.java"
    ...
    processArtifact(className, artifactBaseName.capitalize(), artifactBaseName, 'basilisk-app/' + artifactBaseName + 's')
